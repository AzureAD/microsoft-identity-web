...
### New features
- Add `MicrosoftIdentityOptions.DisableInternalCache` to turn off MSAL's in-memory internal token cache for confidential-client applications, delegating token storage entirely to the configured external cache serializer (or, when none is registered, always fetching from the STS). This surfaces MSAL's `CacheOptions.DisableInternalCache` from [MSAL #5947](https://github.com/AzureAD/microsoft-authentication-library-for-dotnet/pull/5947). It is mutually exclusive with the shared internal cache used by agent applications and by `UseFastUnboundedCache` with the in-memory provider (`IDW10118`).
- Surface MSAL's refresh-token accessor: the `AuthenticationResult` returned by the low-level `ITokenAcquisition` carries the refresh token for the confidential-client flows that expose it (for example long-running On-Behalf-Of), retrievable via MSAL's `AuthenticationResultExtensions.GetRefreshToken()` extension from [MSAL #5947](https://github.com/AzureAD/microsoft-authentication-library-for-dotnet/pull/5947). Combine it with `DisableInternalCache` and `IByRefreshToken.AcquireTokenByRefreshToken` to own refresh-token storage externally.
- mTLS bearer transport now covers the user flows. A certificate credential with `CredentialDescription.UseBoundCredential = true` presents the client certificate over mTLS (`SendCertificateOverMtls`) for On-Behalf-Of and refresh_token token requests — not just the client-credentials (app) flow — routing them to the mTLS token endpoint (`mtlsauth.*`) while the issued token stays a Bearer token. This surfaces [MSAL #6009](https://github.com/AzureAD/microsoft-authentication-library-for-dotnet/pull/6009).
