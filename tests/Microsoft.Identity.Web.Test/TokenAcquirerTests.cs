...
namespace Microsoft.Identity.Web.Test
{
    public class TokenAcquirerTests
    {
        ...
        {
            // Arrange
            const string autoGeneratedKey = "test-auto-generated-key";
            var authResult = CreateMockAuthenticationResult();
            ...
            _tokenAcquisition.GetAuthenticationResultForUserAsync(
                ...
                .Returns(callInfo =>
                {
                    // simulate TokenAcquisition writing the auto-generated key back to the options
                    var options = callInfo.ArgAt<TokenAcquisitionOptions>(5);
                    if (options is not null)
                    {
                        options.LongRunningWebApiSessionKey = autoGeneratedKey;
                    }
            ...
            // Assert
            Assert.Equal(autoGeneratedKey, callerOptions.LongRunningWebApiSessionKey);
        }

        [Fact]
        public async Task GetTokenForUserAsync_WithExplicitSessionKey_PropagatesKeyBackToCaller()
        {
            // Arrange
            const string explicitKey = "test-key";
            var authResult = CreateMockAuthenticationResult();
            var callerOptions = new AcquireTokenOptions
            {
                LongRunningWebApiSessionKey = explicitKey,
            };
            ...
            _tokenAcquisition.GetAuthenticationResultForUserAsync(
                ...
                .Returns(callInfo =>
                {
                    // simulate that TokenAcquisition does not modify the key when an explicit key is provided
                    return Task.FromResult(authResult);
                });
            ...
            // Assert
            Assert.Equal(explicitKey, callerOptions.LongRunningWebApiSessionKey);
        }

        [Fact]
        public async Task GetTokenForUserAsync_WithNoSessionKey_SessionKeyRemainsNull()
        {
            // Arrange
            var authResult = CreateMockAuthenticationResult();
            var callerOptions = new AcquireTokenOptions
            {
                // LongRunningWebApiSessionKey is not set
            };
