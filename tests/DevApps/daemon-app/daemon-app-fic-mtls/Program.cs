...
//    The app uses FIC (signed assertion from managed identity) for
//    client authentication. MSAL provisions the mTLS binding certificate
//    through the V2 credential API.
...
Console.WriteLine("Requesting mTLS-bound token via FIC (signed assertion from MI)...");

// ── 3. call the vault (app-token path with mTLS PoP + FIC) ─────────────
    ...
    && valueElement.ValueKind == JsonValueKind.String)
{
    ...
    Console.WriteLine(!string.IsNullOrEmpty(secret)
        ? "Secret retrieved successfully with FIC + mTLS PoP token (non-null)."
