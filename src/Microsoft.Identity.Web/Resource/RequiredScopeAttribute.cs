...
{
    /// <summary>
    /// This attribute is used on controller, pages, or controller actions
    /// to declare (and validate) the scopes required by a web API. These scopes can be declared
    /// in two ways: hardcoding them, or declaring them in the configuration. Depending on the way
    /// you chose, use either one or the other of the constructors.
    ...
    public class RequiredScopeAttribute : TypeFilterAttribute
    {
        ...
        /// If the token obtained for this API on behalf of the authenticated user does not have
        /// any of these <paramref name="acceptedScopes"/> it its scope claim, the
